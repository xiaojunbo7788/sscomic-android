...
public class MyGlide {
    ...
    public static void GlideImage(Activity activity, int width, int height, BaseComicImage comicImage,
        ...
        if (localPathFile != null) {
            if (localPathFile.exists()) {
                ...
                String url = comicImage.getImage();
                if (url == null || url.length() == 0) {
                    return;
                } else {
                    RequestOptions options = new RequestOptions()
     ...
     * @param imageView
     */
    public static void GlideNewImageNew(Activity activity, int width, int height, BaseComicImage comicImage,
                                     ImageView imageView,String fallbackUrl) {
        File localPathFile = getLocalComicImageFile(comicImage);
        if (localPathFile != null) {
            if (localPathFile.exists()) {
                RequestOptions options = new RequestOptions()
                        .placeholder(R.mipmap.pic_default)        //加载成功之前占位图
                        .error(R.mipmap.pic_default)        //加载错误之后的错误图
                        .skipMemoryCache(false)
                        .override(width, height)
                        .format(DecodeFormat.PREFER_ARGB_8888)
                        .diskCacheStrategy(DiskCacheStrategy.NONE);
